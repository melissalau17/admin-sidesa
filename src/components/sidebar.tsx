         ...
         <nav className="flex-1 space-y-1 p-2 overflow-y-auto">
            {menuItems.map((item) => (
               <Link
                  ...
                  href={item.href}
                  className={cn(
                     "flex items-center rounded-md px-3 py-2.5 text-sm font-medium transition-colors", pathname === item.href
                     ? "bg-[#00695C] text-white" : "text-white hover:bg-[#00695C] hover:text-white"
                  )}
                  onClick={() => setIsOpen(false)} // Close sidebar on mobile after click
                  >
                     <item.icon className="mr-3 h-5 w-5 flex-shrink-0" /> {/* Added flex-shrink-0 */}
                        <span>{item.name}</span> {/* Wrapped name in span for potential future styling */}
               </Link>
                  ))}
               </nav>

               <div className="p-4 mt-auto"> {/* mt-auto to push logout to bottom */}
                  <Button
                     variant="secondary"
